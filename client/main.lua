...
RegisterNetEvent('esx_notifpolisi:outlawNotify')
AddEventHandler('esx_notifpolisi:outlawNotify', function(alert)
...
RegisterNetEvent('esx_notifpolisi:carJackInProgress')
AddEventHandler('esx_notifpolisi:carJackInProgress', function(targetCoords)
	if isPlayerWhitelisted then
...
RegisterNetEvent('esx_notifpolisi:gunshotInProgress')
AddEventHandler('esx_notifpolisi:gunshotInProgress', function(targetCoords)
...
RegisterNetEvent('esx_notifpolisi:combatInProgress')
AddEventHandler('esx_notifpolisi:combatInProgress', function(targetCoords)
	...
	while true do
		...
		if (IsPedTryingToEnterALockedVehicle(playerPed) or IsPedJacking(playerPed)) and Config.CarJackingAlert then
			...
			if vehicle and ((isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted) then
				...
				ESX.TriggerServerCallback('esx_notifpolisi:isVehicleOwner', function(owner)
					if not owner then
						...
						TriggerServerEvent('esx_notifpolisi:carJackInProgress', {
		...
		elseif IsPedInMeleeCombat(playerPed) and Config.MeleeAlert then
			...
			if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
				...
				TriggerServerEvent('esx_notifpolisi:combatInProgress', {
					x = ESX.Math.Round(playerCoords.x, 1),
					y = ESX.Math.Round(playerCoords.y, 1),
					z = ESX.Math.Round(playerCoords.z, 1)
		...
		elseif IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert then
			...
			if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
				...
				TriggerServerEvent('esx_notifpolisi:gunshotInProgress', {
					x = ESX.Math.Round(playerCoords.x, 1),
					y = ESX.Math.Round(playerCoords.y, 1),
					z = ESX.Math.Round(playerCoords.z, 1)
				}, streetName, playerGender)
